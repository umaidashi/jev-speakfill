...
コア `src/core/` は DOM もマイクも知らない。ホストが渡すのは `fields / apply / restore / route` の 4 関数だけなので、React のフォーム state や iOS（`SFSpeechRecognizer` → `/route`）にも同じコアが載る。ドメインの語彙（同義語・単位・数値扱いにする欄名）はコアに固定せず、JSON の設定でホストから注入する。商品登録向けの例が `examples/web-app/speakfill.config.json`。
