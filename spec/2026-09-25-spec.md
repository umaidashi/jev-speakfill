...
> **実装後の注記（2026-09-25 夕方）**: この spec は MVP 着手時の設計。実装中に大きく育った部分は README の「処理の流れ」「アーキテクチャ」が現状の正。主な差分:
> - `core/engine.ts`（状態機械、ホストは 4 関数を DI）と `core/pipeline.ts` を追加。ホストは拡張・web widget・サーバの 3 つ
> - chunk 分割は読点・助詞に加えて `Intl.Segmenter` の単語分割 + 各種の戻し（同スクリプト結合、辞書結合、否定は丸ごと、程度副詞）
> - `core/format.ts` で HTML `type` ごとの正規化・検証（date/time/number/万億/単位…）。旧「B 案 fallback」は不要になった
> - Jev の option 質問は投機的同梱ではなく 2 往復目に分離（トークン 1.4 倍を避ける）。閾値 0.35 → 0.5
> - state に `recent`（直前 3 発話）を追加。発話ごとの `Trace` を保存
> - サンプルフォームはブランドバッグ商品登録（正式英語名の選択肢、状態ランク S–E、金具の色）
